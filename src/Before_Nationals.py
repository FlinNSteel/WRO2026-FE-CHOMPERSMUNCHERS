...
drive = Motor(Port.B, Direction.COUNTERCLOCKWISE) # esta a lreves
...
LIMITE_DER = 30
LIMITE_ROT = 1000
ROT_TIME_MIN = 0000     # Milisegundos mínimos entre giros
MAX_VUELTA = 3
...
def mover_por_mm(distancia_en_mm, rumbo):
    ...
    # Transformación de milímetros a grados del motor (Rueda de 60mm)
    transformacion_grados = (distancia_en_mm * 360) / (50 * 3.1416)
...
def mover_por_tiempo(tiempo):
    reloj_mpt.reset()
    while reloj_rot.time() < tiempo:
        print('tiempo actual:', reloj_mpt.time())
...
def girar(diferencia, med_izq, med_der):
    ...
    # --- TELEMETRÍA --- PARA QUE PUEDA MEDIR LAS COSAS
    print("\n" + "-"*50)
    ...
    if robot_state["giro_direc"] == 1:
        ...
        while yaw_actual > nuevo_objetivo + CORRECCION_GIRO :
            yaw_actual = hub.imu.heading()
            wait(5)
    else:
...
try:
    while robot_state["seccion_actual"] <= SECCIONES_TOTALES:       
        ...
        if Button.CENTER in hub.buttons.pressed():
            ...
            break
        hue, saturation, value = sensor_color.hsv()
        
        # 2. Control Continuo
        mantener_linea_recta(robot_state["rumbo_objetivo"])
        giro_ajuste()
        print ("time is", reloj_rot.time(), )
        # 3. Lectura de Sensores y Decisión
        med_izq, med_der = leer_sensores()

        # Si tenemos lecturas válidas (el búfer se llenó)
        if med_izq is not None:
            diferencia = med_izq - med_der
        if 950 < reloj_rot.time():
            AmIGayQuiz(saturation, hue)
    ...
    reloj_rot.reset() #en este caso es para definir un recto por un ratito, tu entiendes Veronica dw
    print('termina bucle principal, reloj en: ', reloj_rot.time())
    mover_por_tiempo(1500)
    ...
    #Abajo A Izq D Atras B cam Arriba F Cam der C
